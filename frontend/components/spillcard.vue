...
import spill from './cards/spill.vue';
import UserCard from './cards/user.vue';
import { AbbreviateNumber } from '@/composables/abbreviate';
import createIntersectObserver from '@/composables/intersectionObserver'
...
export default defineComponent({
    ...
    components: { Timeago, Item, spill, UserCard }
})
</script>
...
<template>
    <div @mousedown="focused" class="grid p-2 rounded-none bg-theme bg-hover-mute pointer" @click="$router.push({name: 'view-spill', params: { user: user.username, post_id: id}})" v-if="!deleted">
        <spill class="w-full h-full relative">
            <template #avatar>
